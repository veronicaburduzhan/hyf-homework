...
function addStudentToClass(studentName) {
    if (studentName === "")  { //excludes empty string
        return ("Please, add a student name!");
    }

    else if (class07Students.includes(studentName) === true) { //doesn't allow to add the same student
        return ("You cannot add same student.");
    }

    else if (studentName == "Queen") { //allows add Queen even if the class is full
        class07Students.push(studentName);
    } 

    else if (class07Students.length >= 6) { //allows only 6 students in the class
